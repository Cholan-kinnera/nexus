...
async def create_project_service(project_data, owner_id: int, db: AsyncSession):
    ...
    try:
        await create_activity_log(
            ...
            metadata={"project_id": new_project.id, "project_name": new_project.title},
        )
...
async def get_projects_service(
    db: AsyncSession, owner_id: int, params: PaginationParams
):
    query = (
        select(Project)
    ...
    for p in projects:
        logger.info(
            f"Comparing project id={p.id}: current_user.id={owner_id}, project.owner_id={p.owner_id}"
        )
...
async def get_project_by_id_service(project_id: int, current_user, db: AsyncSession):
...
async def update_project_service(
    project_id: int, project_data, current_user, db: AsyncSession
):
    result = await db.execute(select(Project).where(Project.id == project_id))
    ...
    await require_project_role(db, project_id, current_user.id, ["owner", "manager"])
    ...
    try:
        await create_activity_log(
            ...
            metadata={"project_id": project.id, "project_name": project.title},
        )
    except Exception as e:
...
async def delete_project_service(project_id: int, current_user, db: AsyncSession):
    result = await db.execute(select(Project).where(Project.id == project_id))
    ...
    try:
        await create_activity_log(
            ...
            metadata={"project_id": project_id, "project_name": project_title},
        )
    except Exception as e:
