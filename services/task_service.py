...
from services.project_member_service import require_project_role, get_project_member_role
...
# ROLE-BASED VALIDATION: Owner, Manager, Developer can create tasks
    await require_project_role(
        db, task_data.project_id, current_user_id,
        ["owner", "manager", "developer"]
    )

    if task_data.assigned_to is not None:
        user_result = await db.execute(select(User).where(User.id == task_data.assigned_to))
        ...
        assignee_role = await get_project_member_role(db, task_data.project_id, task_data.assigned_to)
    ...
    if new_task.assigned_to is not None:
        try:
            ...
            logger.error(f"Failed to send TASK_ASSIGNED notification or log activity: {e}", exc_info=True)
...
async def update_task_service(
    ...
    if "assigned_to" in update_data and update_data["assigned_to"] is not None:
        user_result = await db.execute(select(User).where(User.id == update_data["assigned_to"]))
        if not user_result.scalar_one_or_none():
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Assigned user not found"
        ...
        # Validate assignee is a project member and not a viewer
        assignee_role = await get_project_member_role(db, task.project_id, update_data["assigned_to"])
        if not assignee_role:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Assigned user is not a member of this project"
    ...
    if task.status != old_status:
        try:
            ...
            logger.error(f"Failed to send task status update notification: {e}", exc_info=True)
    ...
    if task.status == "DONE" and old_status != "DONE":
        try:
            ...
            logger.error(f"Failed to send TASK_COMPLETED notification: {e}", exc_info=True)
