...
class StorageService:
    def __init__(self):
        ...
        logger.info(
            f"  R2_ACCESS_KEY_ID : {'[SET]' if self.access_key_id else '[EMPTY]'}"
        )
        logger.info(
            f"  R2_SECRET_ACCESS : {'[SET]' if self.secret_access_key else '[EMPTY]'}"
        )
        logger.info(f"  R2_BUCKET_NAME   : {self.bucket_name or '[EMPTY]'}")
        logger.info(f"  R2_PUBLIC_URL    : {self.public_url or '[EMPTY]'}")
        logger.info(
            f"  MODE             : {'☁️  CLOUDFLARE R2 (LIVE)' if not self.use_simulator else '💾 LOCAL SIMULATOR'}"
        )
        ...
        if self.use_simulator:
            ...
            try:
                ...
                logger.info(
                    f"  CONNECTION TEST  : ✅ SUCCESS — bucket '{self.bucket_name}' reachable"
                )
    ...
    def validate_file(
        self, file_data: bytes, file_name: str, content_type: Optional[str] = None
    ) -> Tuple[bool, str]:
        """
        ...
        if self.use_simulator:
            try:
                ...
                with open(dest_path, "wb") as f:
                    f.write(file_data)
                logger.info(
                    f"[R2 SIMULATOR] Uploaded file to local storage: {dest_path}"
                )
                return file_key
            except Exception as e:
        ...
        if self.use_simulator:
            try:
                ...
                if os.path.exists(filepath):
                    os.remove(filepath)
                    logger.info(
                        f"[R2 SIMULATOR] Deleted file from local storage: {filepath}"
                    )
                    return True
                logger.warning(
                    f"[R2 SIMULATOR] File key not found for deletion: {file_key}"
                )
                return False
            except Exception as e:
            ...
            try:
                ...
                logger.error(
                    f"Cloudflare R2 delete client error for key {file_key}: {e}"
                )
                return False
            except Exception as e:
                logger.error(
                    f"Cloudflare R2 delete unexpected error for key {file_key}: {e}"
                )
                return False
    ...
    def generate_file_url(self, file_key: str) -> str:
        ...
        base_url = (
            ...
            if self.public_url
            else f"https://{self.bucket_name}.r2.dev"
        )
