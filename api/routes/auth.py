...
async def signup(user: UserSignup, db: AsyncSession = Depends(get_db)) -> AuthResponse:
...
async def login(
    ...
    res = await login_service(user.email, user.password, db)
...
async def verify_otp(
    response: Response, data: VerifyOTPRequest, db: AsyncSession = Depends(get_db)
) -> AuthResponse:
...
async def google_login(
    response: Response, data: GoogleLoginRequest, db: AsyncSession = Depends(get_db)
) -> AuthResponse:
