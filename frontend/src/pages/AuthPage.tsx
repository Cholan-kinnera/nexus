...
const navigate = useNavigate()
  ...
  const switchTab = (tab: "login" | "signup") => setActiveTab(tab);
  ...
  return (
    <div className="min-h-screen bg-[#0a0812] flex items-center justify-center p-4">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-[-10%] right-[-5%] w-[400px] h-[400px] bg-violet-800/8 rounded-full blur-[100px]" />
      </div>

      <div className="relative w-full max-w-[960px] flex rounded-2xl overflow-hidden border border-white/[0.06] shadow-2xl shadow-black/60">

        {/* ── Left Panel ─────────────────────────────────────────────────────── */}
        <LeftPanel activeTab={activeTab} />

        {/* ── Right Panel (sliding forms) ────────────────────────────────────── */}
        <div className="flex-1 bg-[#12101e] flex flex-col p-8 min-h-[580px]">
          ...
          <div className="flex-1 overflow-hidden">
            <div
              className="flex h-full transition-transform duration-[380ms] ease-[cubic-bezier(0.4,0,0.2,1)]"
              style={{
                width: "200%",
                transform:
...
function LeftPanel({ activeTab }: { activeTab: "login" | "signup" }) {
  return (
    <div className="hidden md:flex flex-col w-[420px] bg-[#0e0b1e] p-10 justify-between relative overflow-hidden border-r border-white/[0.05]">
      {/* Decorative grid */}
      <div
      ...
      <div className="relative z-10">
        ...
        {[
          ...
          <div key={f.label} className="flex items-center gap-4 mb-5">
            <div className="w-10 h-10 rounded-xl bg-violet-900/40 border border-violet-800/40 flex items-center justify-center text-base flex-shrink-0">
              {f.icon}
            </div>
            <div>
...
function LoginForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col h-full">
      ...
      {error && (
        <div className="mb-5 px-4 py-3 rounded-lg bg-red-950/60 border border-red-800/50 text-red-400 text-sm">
          {error}
        </div>
      )}
...
function SignupForm({
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4">
        <div className="w-16 h-16 rounded-full bg-violet-600/20 border border-violet-500/30 flex items-center justify-center text-3xl">
          ✅
        </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col h-full">
      ...
      {error && (
        <div className="mb-4 px-4 py-3 rounded-lg bg-red-950/60 border border-red-800/50 text-red-400 text-sm">
          {error}
        </div>
      )}
...
function InputWrap({
  ...
  return (
    <div className="relative">
      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm pointer-events-none opacity-50">
        {icon}
      </span>
      {children}
    </div>
...
function SubmitButton({
  ...
  return (
    <button
      type="submit"
      disabled={loading}
      className="w-full py-2.5 bg-violet-600 hover:bg-violet-500 disabled:opacity-60 disabled:cursor-not-allowed
        text-white text-sm font-medium rounded-lg transition-all duration-200
        flex items-center justify-center gap-2 shadow-lg shadow-violet-900/40 mb-5"
    >
      {loading ? (
...
function Divider() {
  return (
    <div className="relative text-center mb-5">
      ...
      <span className="relative bg-[#12101e] px-3 text-slate-600 text-xs">
        or continue with
      </span>
    </div>
  );
