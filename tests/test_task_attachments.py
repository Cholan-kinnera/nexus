...
class TestTaskAttachments(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.db.add_all([self.owner_user, self.dev_user, self.viewer_user, self.outsider_user])
        ...
        self.outsider_token = create_access_token(data={"sub": self.outsider_user.email})
    ...
    async def cleanup_db(self):
        ...
        for user_email in ["owner@test.com", "dev@test.com", "viewer@test.com", "outsider@test.com"]:
    ...
    async def test_attachment_lifecycle(self):
        async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://127.0.0.1") as client:
            ...
            files_xlsx = {"file": ("sheet.xlsx", xlsx_content, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")}
            response = await client.post(
                f"/api/tasks/{self.task.id}/attachments",
                files=files_xlsx,
                headers=headers
            ...
            response = await client.delete(
                f"/api/tasks/attachments/{csv_attachment_id}",
                headers=headers_owner
            )
            self.assertEqual(response.status_code, status.HTTP_200_OK)
